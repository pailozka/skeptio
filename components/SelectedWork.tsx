...
function ProjectCard({ project }: { project: any }) {
  ...
  return (
    <motion.div
      ...
      onMouseLeave={() => { isHovered.current = false; setCurrentImage(0); }}
    ...
    >
        ...
        {project.images?.map((img: string, idx: number) => (
          <img
            ...
            className={`absolute inset-0 w-full h-full object-cover transition-opacity duration-1000 ${
              idx === currentImage ? "opacity-30 group-hover:opacity-60 z-10" : "opacity-0 z-0"
            }`}
          />
        ))}
        {/* Fallback for single image (if any) */}
      ...
      <div className="relative z-20 flex flex-col items-start gap-4 pointer-events-none">
        ...
        <a
          ...
          className="mt-6 flex items-center gap-2 font-sans font-bold text-[10px] tracking-[0.2em] uppercase border-b border-foreground/20 pb-1 hover:border-foreground transition-all pointer-events-auto"
        >
          VISIT PROJECT
